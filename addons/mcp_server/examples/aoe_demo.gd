...
func _draw() -> void:
	...
	for unit in unit_container.get_children():
		var p_color = players[unit.owner_id]["color"]
		var radius = 7.5 if unit.unit_type == "villager" else 9.5
		...
		if unit.has_meta("attack_flash_t"):
			var flash_t = unit.get_meta("attack_flash_t")
			if flash_t > 0:
				var flash_pos = unit.get_meta("attack_flash_pos")
				draw_line(unit.position, flash_pos, Color(1.0, 0.8, 0.2, flash_t / 0.15), 2.0)
				unit.set_meta("attack_flash_t", flash_t - get_process_delta_time())
